...
import com.oasis.micro.pa.infrastructure.service.PAClient.OrderDetail
import com.oasis.micro.pa.infrastructure.tool.DateTool
import com.oasis.micro.pa.protocol.{PACommand, Pay}
...
case class PAClient(redis: RedisClient)(implicit runtime: ActorRuntime) extends JsonSupport
{
  ...
  {
    response <- Http().singleRequest(HttpRequest(uri = Uri(uri).withQuery(Query(qs: _*))))
    body     <- convert(response).to[JsonObject]
  } yield body
  private[this] def post(uri: String)(entity: Map[String, String]) = for
  {
    ...
    response <- Http().singleRequest(HttpRequest(HttpMethods.POST, Uri(s"${PAClient.gateway}$uri"),
      entity = request).
    withHeaders(RawHeader("Accept", "application/json")))
    body     <- convert(response).to[JsonObject] map (_("data"))
  } yield body
  /**
    * 生成签名
    */
  ...
  //转换支付类型
  private[this] def convertPayType(payType: String)                = (payType: @switch) match
  {
    //微信
    case "3803" => "03"
    //支付宝
  ...
  {
    value  <- redis.get[String](key_access_token)
    result <- value map (Future(_)) getOrElse
    {
  ...
  def upload(r: Pay) =
  {
    ...
    val params = Map(
      ...
      "payType"               -> convertPayType(r.payType),
      //订单状态 02 => 已付款
      "orderStatus"           -> "02",
      //订单详情Uri
    ...
    {
      ac <- getAccessToken
      result <- post(s"/order/postBack/access_token=$ac")(request)
    } yield result
  }
...
{
  private[this] lazy val paConfig = loader.getConfig("pa")
